...
working_dir = r"/var/www/motion"
percent_threshold = 82
extension=(".mp4", ".avi", ".img")
LOGGING = {
    'version': 1,
...
def get_percent_free_disk_space(rootfolder=working_dir):
...
def free_space_up_to(free_percent_required, rootfolder, extension):
    ...
    while file_list:
        if get_percent_free_disk_space() >= free_percent_required:
...
def main(argv=None): 
	...
	if os.path.exists(sys.argv[1]) and os.path.isdir(sys.argv[1]):
	    ...
	    raise ValueError("Directory doesn't exists!!: " + sys.argv[1])
