  ...
  service.files.list({
    ...
    if (files.length == 0) {
      ...
      for (var i = 0; i < files.length; i++) {
        var file = files[i];
        //console.log('%j\n%s\n%s\n', file, file.imageMediaMetadata.width,  file.imageMediaMetadata.height);
	console.log('%s\t%s\t%s\t%s', file.name, file.webContentLink, file.imageMediaMetadata.width,  file.imageMediaMetadata.height);
	//console.log('%s', file.webContentLink);
      }
    }
  });
}
